...
class CNN:
    ...
    def __init__(self, nfilters, sfilters):
        self.model = Sequential([
            ...
            layers.Conv2D(nfilters[0], kernel_size=(sfilters[0], sfilters[0]), activation='relu'),
            layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2)),

            layers.Conv2D(nfilters[1], kernel_size=(sfilters[1], sfilters[1]), padding='same', activation='relu'),
            layers.BatchNormalization(),
            layers.Conv2D(nfilters[1], kernel_size=(sfilters[1], sfilters[1]), padding='same', activation=None),
            layers.BatchNormalization(),
            layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2)),
            ...
            layers.BatchNormalization(),
            layers.Conv2D(nfilters[2], kernel_size=(sfilters[2], sfilters[2]), padding='same', activation=None),
            layers.BatchNormalization(),
            layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2)),
            layers.Activation('relu'),
